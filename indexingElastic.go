...
func indexingWithBulk(index []Indexes, docs []string, URL string, curr int) int {
	...
	if err != nil {
		...
		fmt.Println("curr : indexed = ", curr, " : ",len(indexed))
...
func callIndex(fileName string, numberPerReq int)  {
	...
	for scanner.Scan() {
		...
		if len(docs) % (numberPerReq/2) == 0 {
			fmt.Println("len docs = ", len(docs))
		}
		if len(docs) % numberPerReq == 0  && len(docs) != 0{ // Cứ numberPerReq doc index 1 lần

			fmt.Println("================================ Before", curr)
			fmt.Println("================================ Sending request, pls wait!")
...
func main()  {
	...
	callIndex("shakespeare.json", 10000)
